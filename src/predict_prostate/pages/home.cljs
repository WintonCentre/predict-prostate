     ...
     [:.row
      [:.col-sm-5.col-sm-offset-1
       ...
       [:p {:style {:margin-left 15}}
        [:i (ttt [:home/dymtv "Did you mean to visit"]) " "] [:a {:href "https://breast.predict.cam"} "Predict Breast Cancer"] (ttt [:home/qmark "?"])]
       [:p {:style {:margin-left 15
                    :color "#4c4c6b"}}
         ...
         [:.row {:key 2 :style {:margin-left 0}}
          ...
          [:.col-sm-9 {:key 8 :style {:font-size 16 :margin-left 0}}
           [:a {:href   "https://youtu.be/jCnXBCd13c8"
                :target "_blank"} "PREDICT預報前列線網上工具——幫助初確診前列線癌病患作出醫療決定：https://prostate.predict.cam"]]
          ...
          [:.col-sm-9 {:key 10 :style {:font-size 16 :margin-left 0}}
           [:a {:href   "https://youtu.be/7sGtrnNOWE0"
                :target "_blank"} "PREDICT 预报前列线网上工具--帮助初确诊前列线癌病患作出医疗决定: https://prostate.predict.cam"]]
