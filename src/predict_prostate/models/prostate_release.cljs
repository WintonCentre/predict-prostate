...
(defn pi-pcsm
  ...
  [{:keys [age psa t-stage grade-group primary-rx brca biopsy50 biopsy-done biopsy-cores-involved biopsy-cores-taken] :as inputs}]
  (let [ppc (if (and biopsy-cores-involved biopsy-cores-taken (= 1 biopsy-done))
    ...
    (+ (* 0.0026005 (- (pow (/ age 10) 3) 341.155151))
       ...
       (get {nil 0
             ...
             1 0.956
             2 0} brca)
       (* (- (Math/sqrt (/ (+ ppc 0.1811159) 100)) 0.649019) 1.890134))))
(comment
